...
var MAIN_BRANCH = process.env.MAIN_BRANCH || "main";
var NEW_BRANCH = process.env.NEW_BRANCH || "dev-sam-auto-bumper";
var COMMIT_MESSAGE = process.env.COMMIT_MESSAGE || "[bot] Automatically bump dependencies";
var hasChanges = () => (0, import_child_process.spawnSync)("git", ["status", "--porcelain"], { stdio: "pipe", shell: true }).stdout.toString().trim() !== "";
function runCommand(program, ...programArguments) {
  const programArgumentsQuoted = programArguments.map((it) => it.includes(" ") ? `"${it}"` : it).join(" ");
  console.log(`> ${program} ${programArgumentsQuoted}`);
  const process2 = (0, import_child_process.spawn)(program, programArguments, { stdio: "inherit" });
  return new Promise((resolve) => process2.on("exit", (status) => resolve(status === 0)));
}
async function main() {
  await runCommand("yarn");
  if (!await runCommand("yarn", "bump"))
  ...
  const existingPR = (await octokit.pulls.list({ owner, repo, state: "open" })).data.find((pr) => pr.title === COMMIT_MESSAGE);
  if (existingPR == null) {
